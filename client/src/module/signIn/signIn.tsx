...
const SignIn : React.FC = () => {
    ...
    const data = useAppSelector(state => state.signInReducer.data);
    const loading = useAppSelector(state => state.signInReducer.loading)
    const error = useAppSelector(state => state.signInReducer.error)
    ...
    useEffect(() => {
        if (data) {
            ...
            if (data.err === 2) {
                setnotice({isshow: true, message: data.message, severity: "Thông báo", type: "Notification Just Noitce"})
                setEmail('');
    ...
    return (
        <>
            ...
            <div className={`${styles.containerPage} ${styles[theme_now]}`}>
                ...
                <ModalNotice show={notice.isshow} message={notice.message} severity={notice.severity} type={notice.type} onHide={() => setNoticeHide()}/>
            </div>
        </>
    )
}
